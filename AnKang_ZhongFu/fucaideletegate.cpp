...
fucaideletegate::fucaideletegate(int column1, QObject *parent) :
...
QWidget *fucaideletegate::createEditor(QWidget *parent,const QStyleOptionViewItem &option,const QModelIndex &index) const
{
    if(index.column() == column1){
        QComboBox *editor = new QComboBox(parent);
        QString searchSQL = QString("select MaterialName from AuxiliaryMaterialSetting group by MaterialName;");
    ...
    else if(index.column() == 0 || index.column() == 2 || index.column() == 3 || index.column() == 5 || index.column() == 8){
        return NULL;
    }
    else if(index.column() == 4){
        QComboBox *editor = new QComboBox(parent);

        QString searchSQL = QString("select MaterialSpecifications from AuxiliaryMaterialSetting where MaterialName='%1';")
                .arg(index.model()->data(index.model()->index(index.row(),1),Qt::EditRole).toString());
        db.sql_fillComboBoxItem(searchSQL.toStdString().data(),editor);
        connect(editor, SIGNAL(currentTextChanged(QString)),this, SLOT(commitEditor()));
        return editor;
    }
        ...
        QString searchSQL = QString("select SupplierName from AuxiliaryMaterialSupplier where CommodityCategory='%1';")
                .arg(index.model()->data(index.model()->index(index.row(),2),Qt::EditRole).toString());
        db.sql_fillComboBoxItem(searchSQL.toStdString().data(),editor);
        connect(editor, SIGNAL(currentTextChanged(QString)),this, SLOT(commitEditor()));
        return editor;
    }
...
//设置控件的值
void fucaideletegate::setEditorData(QWidget *editor,const QModelIndex &index) const
{
    if(index.column() == column1){
...
//设置model的值
void fucaideletegate::setModelData(QWidget *editor, QAbstractItemModel *model,const QModelIndex &index) const
{
    if(index.column() == column1){
        QComboBox *comBox = qobject_cast<QComboBox*>(editor);
        ...
        QString fillSQL = QString("select AuxiliaryNumber,GenericClass,SubordinateClass,MaterialSpecifications,Company from \
                                  AuxiliaryMaterialSetting where MaterialName='%1';").arg(str);
        //设置辅材编号
