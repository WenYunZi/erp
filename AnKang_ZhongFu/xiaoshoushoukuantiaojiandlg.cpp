#include "xiaoshoushoukuantiaojiandlg.h"

extern mymysql db;
extern myTCPsocket *sockClient;

xiaoshoushoukuantiaojianDlg::xiaoshoushoukuantiaojianDlg(QWidget *parent) :
    QDialog(parent)
{
    SQL = "SELECT CustomerName 客户名称,ProjectName 工程名称,CollectionTerms 收款条件,ReceivableStatus 应收状态,\
           ContractNumber 合同号 FROM SalesContractManagement;";

    QLabel *label1 = new QLabel(tr("客户名称"),this);
    label1->setAlignment(Qt::AlignVCenter|Qt::AlignRight);
    QLabel *label2 = new QLabel(tr("工程名称"),this);
    label2->setAlignment(Qt::AlignVCenter|Qt::AlignRight);
    QLabel *label3 = new QLabel(tr("应收状态"),this);
    label3->setAlignment(Qt::AlignVCenter|Qt::AlignRight);
    QLabel *label4 = new QLabel(tr("收款条件"),this);
    label4->setAlignment(Qt::AlignVCenter|Qt::AlignRight);

    comBox1 = new QComboBox(this);
    const char *fillSQL1 = "select CustomerName from CustomerFiles;";
    db.sql_fillComboBoxItem(fillSQL1,comBox1);
    connect(comBox1,SIGNAL(currentIndexChanged(int)),this,SLOT(comBox1Correspond()));

    comBox2 = new QComboBox(this);
    comBox2->setEditable(true);

    comBox3 = new QComboBox(this);
    comBox3->setEditable(true);
    comBox3->setView(new QListView());
    comBox3->addItem(tr(""));

    textEdit = new QTextEdit(this);
    textEdit->setFixedHeight(100);

    Btn1 = new QPushButton(tr("确认"),this);
    connect(Btn1,SIGNAL(clicked()),this,SLOT(on_querenBtn()));

    view = new QTableView(this);
    db.showview(SQL,view,&model);
    connect(view,SIGNAL(doubleClicked(QModelIndex)),this,SLOT(ShowSupplyclick()));
    view->setColumnWidth(0,300);
    view->setColumnWidth(1,300);

    QGridLayout *layout = new QGridLayout;
    layout->addWidget(label1,0,0);
    layout->addWidget(comBox1,0,1);
    layout->addWidget(label2,0,2);
    layout->addWidget(comBox2,0,3);
    layout->addWidget(label3,0,4);
    layout->addWidget(comBox3,0,5);
    layout->addWidget(Btn1,0,6,2,1,Qt::AlignVCenter);
    layout->addWidget(label4,1,0);
    layout->addWidget(textEdit,1,1,1,5);
    layout->setColumnStretch(0,0);
    layout->setColumnStretch(1,2);
    layout->setColumnStretch(2,0);
    layout->setColumnStretch(3,2);
    layout->setColumnStretch(4,0);
    layout->setColumnStretch(5,1);
    layout->setColumnStretch(6,1);

    QVBoxLayout *vlayout = new QVBoxLayout(this);
    vlayout->addLayout(layout);
    vlayout->addWidget(view);
}

void xiaoshoushoukuantiaojianDlg::refresh()
{
    db.showview(SQL,view,&model);
}

void xiaoshoushoukuantiaojianDlg::keyPressEvent(QKeyEvent *event)
{
    switch(event->key())
    {
        case Qt::Key_Escape:
           event->ignore();
           break;
        default:
           QDialog::keyPressEvent(event);
    }
}

void xiaoshoushoukuantiaojianDlg::comBox1Correspond()
{
    comBox2->clear();
    QString fillSQL2 = QString("select ProjectName from SalesContractManagement where CustomerName='%1' group by ProjectName;")
            .arg(comBox1->currentText());
    db.sql_fillComboBoxItem(fillSQL2.toStdString().data(),comBox2);
}

void xiaoshoushoukuantiaojianDlg::ShowSupplyclick()
{
    ContractNumber.clear();
    int row = view->currentIndex().row();
    comBox1->setCurrentText(model->item(row,0)->text());
    comBox2->setCurrentText(model->item(row,1)->text());
    comBox3->setCurrentText(model->item(row,3)->text());
    textEdit->setText(model->item(row,2)->text());
    ContractNumber = model->item(row,4)->text();
}

void xiaoshoushoukuantiaojianDlg::on_querenBtn()
{
    QString updateSQL = QString("update SalesContractManagement set CollectionTerms='%1',ReceivableStatus='%2' where ContractNumber='%3'")
            .arg(textEdit->toPlainText(),comBox3->currentText(),ContractNumber);
    int res = db.sql_exec(updateSQL.toStdString().data());
    if(res == 0){
        comBox1->setCurrentIndex(0);
        comBox2->setCurrentIndex(0);
        comBox3->setCurrentIndex(0);
        textEdit->clear();
        sockClient->send_Msg3();
        QMessageBox::information(this,tr("提示信息"),tr("确认成功"),0,0,0);
    }
}
